...
| 경로 | 내용 |
| --- | --- |
| `/partners/` | 전사 협력사 리스트. 종합 신호등·등급, 카테고리별 신호등, 6개월 추세, 최종 갱신일. 검색·신호등·등급·담당자 필터와 정렬(위험 우선 기본) |
| `/partners/<id>` | 협력사 상세. 카테고리별 지표 값·점수·스파크라인, 즉시위험 사유, 자동 감점 내역, 리스크 이벤트 타임라인 |
| `/partners/settings` | 지표 정의(가중치·good/bad 기준·사용 여부) 편집, **지표 추가**, 수집기 상태, 수집 로그 |
